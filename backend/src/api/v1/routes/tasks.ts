...
export const createTaskRouter = (): Router => {
  ...
  router.get('/:taskId', controller.getTaskById.bind(controller))

  router.post(
    '/',
    [
      body('boardId').notEmpty().trim().withMessage('boardId is required'),
  ...
  router.delete('/:taskId', controller.deleteTask.bind(controller))
