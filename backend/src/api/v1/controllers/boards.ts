...
export class BoardController {
  ...
  public async getBoardTasks(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      req?.log?.error(
        `Failed to fetch tasks for board with id ${req.params.boardId}`,
        { error },
      )
  ...
  public async getBoardById(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const board = await this.boardService.getBoardById(req, {
        id: boardId as string,
      })

      res.status(StatusCodes.SUCCESS).json({ data: board })
    } catch (error) {
      req?.log?.error(`Failed to fetch board with id ${req.params.boardId}`, {
        error,
      })
  ...
  public async updateBoard(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const updatedBoard = await this.boardService.updateBoard(req, {
        id: boardId as string,
        boardData,
      })
      ...
      req?.log?.error(`Failed to update board with id ${req.params.boardId}`, {
        error,
      })
  ...
  public async deleteBoard(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      req?.log?.error(`Failed to delete board with id ${req.params.boardId}`, {
        error,
      })
